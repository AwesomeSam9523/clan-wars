...
staff = [646508657655283721, 753869126522503310, 471174028212568064, 771601176155783198, 482612710018908190, 660353231565619200, 306582372034281484, 504029508295196683, 842906181370380288]
        ...
        if vids.get('error'):
            ...
            if err['code'] == 403:
                await bot.get_channel(867264621211156500).send(
                    ...
                    f'Reqs: {reqs}. API Index: {SOCIAL_KEYS.index(bot.apikey)}'
                )
                if reqs >= 2500 and bot.apikey == YT_API_KEY:
                    reqs = 12475
                elif reqs >= 12475 and bot.apikey == SOCIAL_KEYS[0]:
